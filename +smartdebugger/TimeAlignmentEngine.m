classdef TimeAlignmentEngine
    ...
    methods (Static)
        function [t,a,b] = align(aTime,aData,bTime,bData,method)
            ...
            aData = TimeAlignmentEngine.selectFirstDimension(aData,ia);
            bData = TimeAlignmentEngine.selectFirstDimension(bData,ib);
            ...
            a = [];
            b = [];
            if numel(aTime) == 1, a = TimeAlignmentEngine.repeatData(aData,numel(t)); end
            if numel(bTime) == 1, b = TimeAlignmentEngine.repeatData(bData,numel(t)); end
            if isempty(a), a = TimeAlignmentEngine.interpolate(aTime,aData,t,method); end
            if isempty(b), b = TimeAlignmentEngine.interpolate(bTime,bData,t,method); end
        end
    end

    methods (Static, Access=private)
        function out = interpolate(t,data,newT,method)
        ...
        function data = selectFirstDimension(data,index)
        ...
        function out = repeatData(data,n)
