classdef SmartDebuggerApp < handle
    %SMARTDEBUGGERAPP Smart Debugger UI for Simulink/TargetLink MIL and SIL.
    ...
    methods
        ...
        function configure(obj,varargin)
            ...
            s = char(string(p.Results.SILModel));
        ...
        function runDebug(obj)
            ...
            obj.setBusy(true);
            c1 = onCleanup(@()obj.restoreRunSelection(snapshot)); %#ok<NASGU>
            c2 = onCleanup(@()obj.setBusy(false)); %#ok<NASGU>
            try
                ...
                if strcmpi(obj.Mode,'MIL')
                    ...
                    obj.status(['Running MIL simulation for ' obj.RunTargetBlock ' ...']);
                    drawnow;
                    ...
                    if isempty(silBlock)
                        ...
                        if isempty(strtrim(override))
                            error('SmartDebugger:SILMappingRequired','Automatic MIL-to-SIL mapping failed. Enter the SIL block override.');
                        end
                        silBlock = override;
    ...
    methods (Access=private)
        ...
        function buildCenterPanel(obj,parent)
            ...
            g=uigridlayout(p,[5 1]); g.RowHeight={'1x','1x','1x','1x',30};
        ...
        function buildPlotPanel(obj,parent)
            p=uipanel(parent,'Title','Runtime Trace | actual logged samples'); p.Layout.Row=3; p.Layout.Column=[1 3];
            obj.PlotAxes=uiaxes(p,'Position',[10 10 1520 260]); title(obj.PlotAxes,'Selected runtime signal'); xlabel(obj.PlotAxes,'Time (s)'); ylabel(obj.PlotAxes,'Value'); grid(obj.PlotAxes,'on');
        end
        ...
        function addTreeChildren(obj,parentNode,parentPath)
            ...
            for k=1:numel(children)
                ...
                if strcmp(childPath,parentPath), continue; end
                try, name=get_param(childPath,'Name'); catch, name=childPath; end
                node=uitreenode(parentNode,'Text',name,'NodeData',childPath);
                try
                    if strcmp(get_param(childPath,'BlockType'),'SubSystem'), obj.addTreeChildren(node,childPath); end
                catch
                end
            end
        end
        ...
        function treeSelectionChanged(obj,event)
            try
                ...
                if ischar(path)||isstring(path), obj.attachToBlock(char(string(path)),true); end
        ...
        function data=inspectionTableData(~,ports)
            ...
            for k=1:numel(ports), data{k,1}=ports(k).Port; data{k,2}=ports(k).Name; data{k,4}=ports(k).DataType; data{k,5}=ports(k).Dimension; data{k,6}=ports(k).SampleTime; end
        end
        function populateBlockInfo(obj,info)
            ...
            if isfield(info,'IsStateflow')&&info.IsStateflow, lines{end+1}='Stateflow: supported object detected'; end
        ...
        function data=runtimeTableData(obj,ports)
            data=cell(numel(ports),6);
            for k=1:numel(ports), data{k,1}=ports(k).Port; data{k,2}=ports(k).Name; data{k,3}=obj.formatValue(ports(k).Value); data{k,4}=ports(k).DataType; data{k,5}=ports(k).Dimension; data{k,6}=ports(k).SampleTime; end
        end
        ...
        function runtimeSelection(obj,~,event,direction)
            try
                ...
                row=event.Indices(1); result=obj.ActiveResult; if isempty(result), return; end
                if strcmpi(direction,'Input'), ports=result.Inputs; else, ports=result.Outputs; end
                if row<1||row>numel(ports), return; end
                obj.plotRuntimeSignal(ports(row)); obj.populateSampleTable(ports(row));
        ...
        function plotDefaultRuntimeTrace(obj,result)
            ...
            for k=1:numel(result.Inputs)
                ...
                try, [t,y]=obj.seriesXY(result.Inputs(k).Series); if ~isempty(t), obj.plotSampleAccurate(t,y); names{end+1}=result.Inputs(k).Name; plotted=true; end; catch ME, obj.DiagnosticsManager.recordException(ME,['Plot ' result.Inputs(k).Name]); end
            end
            for k=1:numel(result.Outputs)
                if isempty(result.Outputs(k).Series), continue; end
                try, [t,y]=obj.seriesXY(result.Outputs(k).Series); if ~isempty(t), obj.plotSampleAccurate(t,y); names{end+1}=result.Outputs(k).Name; plotted=true; end; catch ME, obj.DiagnosticsManager.recordException(ME,['Plot ' result.Outputs(k).Name]); end
            end
            hold(obj.PlotAxes,'off');
            if plotted, legend(obj.PlotAxes,names,'Interpreter','none','Location','best'); title(obj.PlotAxes,'Actual logged samples, first timestamp through last timestamp'); else, title(obj.PlotAxes,'No runtime series captured'); end
        end
        ...
        function plotSampleAccurate(obj,t,y)
            t=t(:); y=y(:); if isempty(t)||isempty(y), return; end
            n=min(numel(t),numel(y)); t=t(1:n); y=y(1:n); regular=false;
            if n>=2, d=diff(t); m=median(d); regular=all(abs(d-m)<=max(1e-10,1e-8*max(abs(m),1))); end
            if islogical(y), yp=double(y); else, yp=y; end
            if regular, stairs(obj.PlotAxes,t,yp,'Marker','.','LineWidth',1); else, plot(obj.PlotAxes,t,yp,'Marker','.','LineStyle','-'); end
        ...
        function ensureSelectedBlock(obj)
            path=char(string(obj.RunTargetBlock)); if isempty(strtrim(path)), path=char(string(obj.SelectedBlock)); end; if isempty(strtrim(path)), path=char(string(obj.BlockField.Value)); end; if isempty(strtrim(path)), path=obj.ModelManager.currentSimulinkSelection(); end
            ...
            obj.SelectedBlock=path; obj.BlockField.Value=path; obj.RunTargetBlock=path;
        end
        function validateRunInputs(obj)
            if isempty(strtrim(obj.RunTargetBlock)), error('SmartDebugger:MissingBlock','Select and import a Simulink block first.'); end
            s=strtrim(char(string(obj.StopTimeField.Value)));
            if isempty(s)||strcmpi(s,'auto')||strcmpi(s,'auto (model)'), if strcmpi(obj.Mode,'SIL')&&isempty(strtrim(char(string(obj.SILModelField.Value)))), error('SmartDebugger:MissingSILModel','Select a SIL model first.'); end; return; end
        ...
        function model=resolveMILModel(obj)
            root=bdroot(obj.RunTargetBlock); if ~isempty(root)&&bdIsLoaded(root), model=root; else, model=char(string(obj.MILModelField.Value)); end
            if isempty(strtrim(model)), error('SmartDebugger:MissingMILModel','No MIL model could be resolved.'); end
        end
        function restoreRunSelection(obj,s)
            try, obj.SelectedBlock=s.Block; obj.BlockField.Value=s.BlockField; obj.MILModelField.Value=s.MILModel; obj.SelectedSILBlock=s.SILBlock; obj.SILBlockField.Value=s.SILBlockField; obj.RunTargetBlock=s.Block; catch, end
        end
        function setBusy(obj,value)
            obj.Busy=logical(value); try, if value, obj.ModeDropDown.Enable='off'; obj.StopTimeField.Enable='off'; else, obj.ModeDropDown.Enable='on'; obj.StopTimeField.Enable='on'; end; catch, end
        end
        ...
        function watchSelection(obj)
            if obj.Busy, return; end
            try, path=obj.ModelManager.currentSimulinkSelection(); if isempty(path)||strcmp(path,obj.SelectedBlock), return; end; obj.attachToBlock(path,true); catch, end
        end
