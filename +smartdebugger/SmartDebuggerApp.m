classdef SmartDebuggerApp < handle
    ...
    methods
        ...
        function setMILModel(obj,model)
            try
                ...
                obj.SelectedBlock=''; obj.RunTargetBlock='';
        ...
        function setSILModel(obj,model)
            try
                ...
                if ~bdIsLoaded(root)
                    if isempty(ext)
                        if exist([root '.slx'],'file')==2 || exist([root '.mdl'],'file')==2, load_system(root);
                        else, error('SmartDebugger:SILModelNotFound','SIL model not found: %s',model); end
        ...
        function inspectBlock(obj)
            ...
            try
                ...
                obj.SelectedBlock=path; obj.RunTargetBlock=path;
                obj.populatePorts(info); obj.populateBlockInfo(info);
                obj.status(['Inspected: ' path]); obj.showDiagnostics();
            catch ME, obj.handleError(ME,'Inspect block'); end
        end
        function runDebug(obj)
            if obj.Busy, return; end
            try
                obj.ensureSelectedBlock(); obj.validateRunInputs();
                obj.setBusy(true); c=onCleanup(@()obj.setBusy(false)); %#ok<NASGU>
                ...
                if strcmpi(obj.Mode,'MIL')
                    ...
                    if isempty(silBlock)
                        ...
                        if isempty(strtrim(override)), error('SmartDebugger:SILMappingRequired','Automatic MIL-to-SIL mapping failed. Enter a SIL block override.'); end
                    ...
                    obj.SelectedSILBlock=silBlock; obj.SILBlockField.Value=silBlock;
                    obj.status(['Running SIL: ' silBlock]); drawnow;
                    runResult=obj.SimulationManager.runSIL(silModel,silBlock,stopTime);
                    runResult.MappingConfidence=confidence; runResult.MappingMethod=mapMethod;
                    obj.SILResult=runResult;
                end
                obj.ActiveResult=runResult;
                obj.displayRuntimeResult(runResult); obj.plotDefaultRuntimeTrace(runResult); obj.showDiagnostics();
        ...
        function compare(obj)
            try
                ...
                if strcmp(report.Status,'PASS'), obj.FirstDivergenceLabel.Text='MIL vs SIL: PASS | No mismatch outside tolerance';
                    ...
                    else, obj.FirstDivergenceLabel.Text=sprintf('FIRST DIVERGENCE: %s / Port %d at t = %.12g s',fd.Direction,fd.Port,fd.Time); end
                end
    ...
    methods (Access=private)
        ...
        function buildToolbar(obj,parent)
            ...
            obj.StopTimeField=uieditfield(g,'text','Value','auto','Placeholder','auto = model StopTime'); uilabel(g,'Text','Stop time'); obj.StatusLabel=uilabel(g,'Text','Ready'); obj.StatusLabel.Layout.Row=2; obj.StatusLabel.Layout.Column=[1 12];
        end
        function buildLeftPanel(obj,parent)
            ...
            uilabel(g,'Text','Complete model hierarchy'); obj.Tree=uitree(g,'SelectionChangedFcn',@(~,e)obj.treeSelectionChanged(e)); uibutton(g,'Text','Refresh model tree','ButtonPushedFcn',@(~,~)obj.refreshModelTree());
            uilabel(g,'Text','MIL selected block'); obj.BlockField=uieditfield(g,'text','Placeholder','model/subsystem/block'); uibutton(g,'Text','Open / Highlight','ButtonPushedFcn',@(~,~)obj.navigateToBlock());
            uilabel(g,'Text','SIL mapped block (optional override)'); obj.SILBlockField=uieditfield(g,'text','Placeholder','SIL model/subsystem/block'); uibutton(g,'Text','Inspect selected block','ButtonPushedFcn',@(~,~)obj.inspectBlock());
        end
        function buildCenterPanel(obj,parent)
            ...
            obj.SampleTable=uitable(g,'ColumnName',{'Time (s)','Value'},'RowName',{},'ColumnEditable',false(1,2)); obj.FirstDivergenceLabel=uilabel(g,'Text','No MIL/SIL comparison run yet','FontWeight','bold');
        end
        ...
        function treeSelectionChanged(obj,event)
            try
                nodes=event.SelectedNodes; if isempty(nodes), return; end; path=nodes(1).NodeData;
                if ischar(path)||isstring(path), obj.selectBlock(char(string(path)),false); end
            catch ME, obj.handleError(ME,'Tree selection'); end
        end
        function selectBlock(obj,path,inspectNow)
            ...
            obj.ModelManager.loadModel(root); obj.syncMILModelFromBlock(path); obj.SelectedBlock=path; obj.RunTargetBlock=path; obj.BlockField.Value=path;
        ...
        function runtimeSelection(obj,~,event,direction)
            try
                if isempty(event.Indices)||isempty(obj.ActiveResult), return; end; row=event.Indices(1); if strcmpi(direction,'Input'), ports=obj.ActiveResult.Inputs; else, ports=obj.ActiveResult.Outputs; end
                if row>=1&&row<=numel(ports), obj.plotRuntimeSignal(ports(row)); obj.populateSampleTable(ports(row)); end
            catch ME, obj.handleError(ME,'Plot runtime signal'); end
        end
        ...
        function plotRuntimeSignal(obj,port)
            ...
            try, [t,y]=obj.seriesXY(port.Series); obj.plotSampleAccurate(t,y); title(obj.PlotAxes,sprintf('%s | %d actual samples',port.Name,numel(t))); catch ME, obj.handleError(ME,'Plot runtime signal'); end
        end
        function plotSampleAccurate(obj,t,y)
            t=t(:); y=y(:); n=min(numel(t),numel(y)); if n==0, return; end; t=t(1:n); y=y(1:n); if islogical(y), y=double(y); end
            regular=false; if n>=2, d=diff(t); m=median(d); regular=all(abs(d-m)<=max(1e-10,1e-8*max(abs(m),1))); end
            if regular, stairs(obj.PlotAxes,t,y,'Marker','.','LineWidth',1); else, plot(obj.PlotAxes,t,y,'Marker','.','LineStyle','-'); end
            if n>1, xlim(obj.PlotAxes,[t(1) t(end)]); else, xlim(obj.PlotAxes,[t(1)-0.5 t(1)+0.5]); end; xlabel(obj.PlotAxes,'Time (s)'); grid(obj.PlotAxes,'on');
        end
        ...
        function ensureSelectedBlock(obj)
            path=strtrim(char(string(obj.RunTargetBlock))); if isempty(path), path=strtrim(char(string(obj.SelectedBlock))); end; if isempty(path), path=strtrim(char(string(obj.BlockField.Value))); end
            if isempty(path), error('SmartDebugger:MissingBlock','Select a Simulink block first.'); end; try, get_param(path,'Handle'); catch, error('SmartDebugger:InvalidBlock','Selected block is invalid: %s',path); end
            obj.SelectedBlock=path; obj.RunTargetBlock=path; obj.BlockField.Value=path;
        end
        function validateRunInputs(obj)
            obj.ensureSelectedBlock(); s=strtrim(char(string(obj.StopTimeField.Value))); if isempty(s), obj.StopTimeField.Value='auto'; s='auto'; end
            if ~strcmpi(s,'auto'), v=str2double(s); if ~isscalar(v)||~isfinite(v)||v<0, error('SmartDebugger:InvalidStopTime','Stop time must be a nonnegative number or auto.'); end, end
