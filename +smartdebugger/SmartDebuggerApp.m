classdef SmartDebuggerApp < handle
    ...
    methods
        ...
        function configure(obj,varargin)
            ...
            parse(p,varargin{:});
            if strlength(string(p.Results.Model)) > 0
                obj.setMILModel(p.Results.Model);
            end
            if strlength(string(p.Results.SILModel)) > 0
                obj.setSILModel(p.Results.SILModel);
            end
        end
        ...
        function setSILModel(obj,model)
            try
                ...
                if isempty(ext) && ~bdIsLoaded(root)
                    load_system(root);
                elseif ~isempty(ext)
                    load_system(model);
                end
                ...
                if obj.Mode == "MIL"
                    ...
                    if isempty(silBlock)
                        if strlength(string(obj.SILBlockField.Value)) > 0
                            ...
                            error('SmartDebugger:SILMappingRequired','Automatic MIL-to-SIL mapping failed. Specify the SIL block path manually.');
                        end
                    end
    ...
    methods (Access=private)
        ...
        function buildPlotPanel(obj,parent)
            ...
            ylabel(obj.PlotAxes,'Value / Error');
        ...
        function modeChanged(obj,s)
            obj.Mode = string(s.Value);
            obj.status("Mode: " + obj.Mode);
        end

        function validateRunInputs(obj)
            if isempty(strtrim(string(obj.MILModelField.Value)))
                error('SmartDebugger:MissingMILModel','Select a MIL model first.');
            end
            if isempty(strtrim(string(obj.BlockField.Value)))
                error('SmartDebugger:MissingBlock','Select or enter a MIL block path first.');
            end
            stopTime = str2double(obj.StopTimeField.Value);
            if isnan(stopTime) || ~isfinite(stopTime) || stopTime < 0
                error('SmartDebugger:InvalidStopTime','Stop time must be a finite nonnegative number.');
            end
            if obj.Mode == "SIL" && isempty(strtrim(string(obj.SILModelField.Value)))
        ...
        function displayRuntimeResult(obj,result)
            ...
            if isfield(result,'Status') && strcmp(result.Status,'PARTIAL')
                obj.status('Simulation completed with partial capture. See Diagnostics.');
            end
        end
        ...
        function showScaling(obj,p)
            try
                info = smartdebugger.ScalingInspector.inspect(p.Value);
                lines = {['Signal: ' char(p.Name)],['Class: ' char(info.Class)]};
                if ~isempty(info.WordLength), lines{end+1} = ['Word length: ' char(info.WordLength)]; end
                if ~isempty(info.FractionLength), lines{end+1} = ['Fraction length: ' char(info.FractionLength)]; end
                if ~isempty(info.Slope), lines{end+1} = ['Slope: ' char(info.Slope)]; end
                if ~isempty(info.Bias), lines{end+1} = ['Bias: ' char(info.Bias)]; end
                if ~isempty(info.Message), lines{end+1} = char(info.Message); end
                obj.BlockInfoArea.Value = lines;
            catch ME
                obj.DiagnosticsManager.recordException(ME,'Scaling inspection');
                obj.showDiagnostics();
            end
        end
        ...
        function plotSingleRuntimeSeries(obj,p,titleText)
            ...
            try
                ...
                if isnumeric(d) || islogical(d)
                    if isvector(d)
                        plot(obj.PlotAxes,t,d,'DisplayName',titleText);
                    else
                        plot(obj.PlotAxes,t,d(:,1),'DisplayName',titleText);
                    end
                    title(obj.PlotAxes,titleText);
                    xlabel(obj.PlotAxes,'Time (s)');
                    grid(obj.PlotAxes,'on');
                ...
                obj.DiagnosticsManager.recordException(ME,'Runtime plot');
                obj.showDiagnostics();
            end
        end
        ...
        function populateBlockInfo(obj,info)
            ...
            if isfield(info,'LibraryLink') && ~isempty(info.LibraryLink), lines{end+1} = ['Library: ' char(info.LibraryLink)]; end
            if isfield(info,'MaskType') && ~isempty(info.MaskType), lines{end+1} = ['Mask: ' char(info.MaskType)]; end
            if isfield(info,'LinkStatus') && ~isempty(info.LinkStatus), lines{end+1} = ['Link status: ' char(info.LinkStatus)]; end
            if isfield(info,'IsStateflow') && info.IsStateflow
                lines{end+1} = 'Stateflow: detected';
                sf = info.StateflowInfo;
                if ~isempty(sf.Name), lines{end+1} = ['Chart: ' char(sf.Name)]; end
                if ~isempty(sf.ActionLanguage), lines{end+1} = ['Action language: ' char(sf.ActionLanguage)]; end
                if ~isempty(sf.StateMachineType), lines{end+1} = ['State machine: ' char(sf.StateMachineType)]; end
            end
            obj.BlockInfoArea.Value = lines;
        end
        ...
        function plotComparison(obj,report)
            ...
            obj.plotSeries(obj.PlotAxes,report.Time,report.MIL,'MIL');
            obj.plotSeries(obj.PlotAxes,report.Time,report.SIL,'SIL');
            obj.plotSeries(obj.PlotAxes,report.Time,report.Error,'MIL - SIL');
            hold(obj.PlotAxes,'off');
            legend(obj.PlotAxes,'show','Location','best');
            title(obj.PlotAxes,['Comparison: ' char(report.Signal)]);
            grid(obj.PlotAxes,'on');
        end

        function refreshModelTree(obj)
            delete(obj.Tree.Children);
            model = strtrim(string(obj.MILModelField.Value));
            if model == "", return; end
            try
                [~,root,~] = fileparts(char(model));
                if ~bdIsLoaded(root), load_system(root); end
                rootNode = uitreenode(obj.Tree,'Text',root,'NodeData',root);
                children = find_system(root,'SearchDepth',1,'Type','Block');
                children = children(~strcmp(children,root));
                for k = 1:numel(children)
                    uitreenode(rootNode,'Text',get_param(children{k},'Name'),'NodeData',children{k});
                end
                expand(rootNode);
            catch ME
                obj.DiagnosticsManager.recordException(ME,'Model tree');
                obj.showDiagnostics();
            end
        end
        ...
        function startSelectionWatcher(obj)
            try
                obj.SelectionTimer = timer('ExecutionMode','fixedSpacing','Period',0.75, ...
                    'BusyMode','drop','TimerFcn',@(~,~)obj.pollSelection());
        ...
        function updateCompatibility(obj)
            ...
            if ~isempty(c.Simulink), parts{end+1} = ['Simulink ' char(c.Simulink)]; end
            if c.StateflowAvailable, parts{end+1} = 'Stateflow available'; else, parts{end+1} = 'Stateflow unavailable'; end
            if c.SimulinkCoder, parts{end+1} = 'Simulink Coder available'; end
            if c.TargetLinkAvailable, parts{end+1} = 'TargetLink detected'; end
        ...
        function handleError(obj,ME,stage)
            ...
            obj.status([stage ' failed. See Diagnostics.']);
            obj.showDiagnostics();
        end

        function status(obj,msg)
            if ~isempty(obj.StatusLabel) && isvalid(obj.StatusLabel), obj.StatusLabel.Text = char(msg); end
        end

        function setBusy(obj,value)
            obj.Busy = value;
            if ~isempty(obj.UIFigure) && isvalid(obj.UIFigure)
                if value, obj.UIFigure.Pointer = 'watch'; else, obj.UIFigure.Pointer = 'arrow'; end
                drawnow;
            end
        end
            ...
            try
                ...
                elseif isnumeric(value) || islogical(value)
                    ...
                    if numel(value) <= 8
                        ...
                        text = sprintf('%s %s, min=%s, max=%s',class(value),mat2str(sz), ...
                            mat2str(min(value(:)),8),mat2str(max(value(:)),8));
                    end
                else
                    text = ['<' class(value) '>'];
                end
        ...
        function plotSeries(~,ax,t,data,name)
            if isempty(data), return; end
            try
                if isvector(data)
                    plot(ax,t,data,'DisplayName',name);
                elseif isnumeric(data)
                    plot(ax,t,data(:,1),'DisplayName',name);
                end
            catch
            end
        end
