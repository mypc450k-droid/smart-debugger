    ...
    methods
        ...
        function loadModel(obj,model)
            ...
            try
                ...
                if isempty(ext)
                    if ~bdIsLoaded(root)
                        if exist([root '.slx'],'file') || exist([root '.mdl'],'file'), load_system(root);
                        else, error('SmartDebugger:ModelNotFound','Model not found: %s',model); end
                    end
        ...
        function path=currentSimulinkSelection(obj)
            ...
            try
                ...
                if ~isempty(selected), path=getfullname(selected(1)); return; end
                try
                    selected=get_param(0,'SelectedBlocks');
                    if ischar(selected) && ~isempty(selected), path=selected;
                    elseif iscell(selected) && ~isempty(selected), path=char(selected{1}); end
                catch
                end
            catch ME
        ...
        function ports=portInfo(obj,path,direction) %#ok<INUSD>
            ...
            try
                ...
                if strcmpi(direction,'Inport'), hs=ph.Inport; displayDirection='Input';
                else, hs=ph.Outport; displayDirection='Output'; end
