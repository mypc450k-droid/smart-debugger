    ...
    methods (Access=private)
        function result=run(obj,model,block,stopTime,mode)
            ...
            try
                ...
                inNames=obj.instrumentPorts(block,'Inport',tx);
                outNames=obj.instrumentPorts(block,'Outport',tx);
                oldSL=get_param(model,'SignalLogging');
                tx.record(model,'SignalLogging',oldSL); set_param(model,'SignalLogging','on');
                oldNameMode=get_param(model,'SignalLoggingNameMode');
                tx.record(model,'SignalLoggingNameMode',oldNameMode); set_param(model,'SignalLoggingNameMode','UserSpecified');
                ...
                result.Inputs=obj.readLogged(logs,inNames);
                result.Outputs=obj.readLogged(logs,outNames);
                [result.Time,result]=obj.populateTime(result);
        ...
        function names=instrumentPorts(obj,block,direction,tx) %#ok<INUSD>
            names={};
            ...
            for k=1:numel(hs)
                ...
                oldLog=get_param(line,'DataLogging');
                oldMode=get_param(line,'DataLoggingNameMode');
                oldName=get_param(line,'DataLoggingName');
                tx.record(line,'DataLogging',oldLog); tx.record(line,'DataLoggingNameMode',oldMode); tx.record(line,'DataLoggingName',oldName);
                nm=get_param(line,'Name');
                if isempty(nm), nm=sprintf('SmartDebugger_%s_%d',lower(direction),k); end
                nm=matlab.lang.makeValidName(nm);
                set_param(line,'DataLoggingNameMode','Custom');
                set_param(line,'DataLoggingName',nm);
                set_param(line,'DataLogging','on');
                names{end+1}=nm; %#ok<AGROW>
            end
        end
        function ports=readLogged(obj,logs,names) %#ok<INUSD>
            ...
            for k=1:numel(names)
                ...
                try
                    el=logs.getElement(names{k});
                    ts=el.Values;
                    p.Series=ts;
                    p.Name=names{k};
                    if isprop(ts,'Data')
                        p.Value=ts.Data(end,:,:,:);
                        p.DataType=class(ts.Data);
                        p.Dimension=mat2str(size(ts.Data));
                    end
                    if isprop(ts,'Time') && ~isempty(ts.Time), p.SampleTime='logged'; end
                catch ME
                    obj.Diagnostics.recordException(ME,['Read logged signal ' names{k}]);
                end
        ...
        function [t,result]=populateTime(~,result)
            ...
            for k=1:numel(allPorts)
                s=allPorts(k).Series;
                try
                    if ~isempty(s) && isprop(s,'Time'), t=s.Time; break; end
                catch
                end
            end
            result.Time=t;
