    ...
    methods (Access = private)
        function result = run(obj, model, block, stopTime, mode)
            ...
            result = struct('Mode',mode,'Model',model,'Block',block,'Time',[], ...
                'Inputs',obj.emptyPorts(),'Outputs',obj.emptyPorts(), ...
                'SimulationOutput',[],'Status','FAILED','Message','');
            simOut=[]; logs=[]; inputs=obj.emptyPorts(); outputs=obj.emptyPorts(); root=''; logName='logsout';
            ...
            try
                ...
                if isempty(strtrim(char(string(oldLogName)))), oldLogName='logsout'; end
                ...
                if ~obj.isAutoStopTime(stopTime), simArgs(end+1:end+2)={'StopTime',stopTime}; end
                if strcmpi(mode,'MIL'), simArgs(end+1:end+2)={'CaptureErrors','on'}; end
                obj.Diagnostics.record('INFO',[mode ' simulation'],sprintf('Starting simulation. Model=%s, Block=%s, StopTime=%s, SignalLogging=%s',root,block,stopTime,logName),'SmartDebugger:SimulationStart');
                ...
                if isempty(simError)
                    ...
                    obj.Diagnostics.record('ERROR',[mode ' simulation'],result.Message,'SmartDebugger:SimulationCapturedError');
                end
                if isempty(logs)
                    if isempty(simError)
                        result.Status='PARTIAL'; result.Message='Simulation completed but no signal-log Dataset was returned.';
                ...
                elseif captured<requested
                    result.Status='PARTIAL';
                    result.Message=strtrim(sprintf('%s Captured %d of %d selected top-level ports.',result.Message,captured,requested));
                    obj.Diagnostics.record('WARNING',[mode ' capture'],result.Message,'SmartDebugger:PartialPortData');
                end
            catch ME
                result.SimulationOutput=simOut; result.Status='FAILED'; result.Message=ME.message;
                obj.Diagnostics.recordException(ME,[mode ' simulation']);
            end
        end

        function ports=emptyPorts(~)
            template=struct('Port',0,'Name','','LogName','','Value',[],'DataType','','Dimension','', ...
                'SampleTime','','Series',[],'LineHandle',-1,'SignalHandle',-1,'LoggingHandle',-1);
            ports=repmat(template,0,1);
        end

        function root=ensureModelLoaded(~,model,block)
            root=''; try, root=bdroot(block); catch, end
            if ~isempty(root), try, if bdIsLoaded(root), return; end, catch, end, end
            [~,modelRoot,ext]=fileparts(model); if isempty(modelRoot), modelRoot=model; end
            if bdIsLoaded(modelRoot), root=modelRoot; return; end
            if isempty(ext)
                if exist([modelRoot '.slx'],'file')==2 || exist([modelRoot '.mdl'],'file')==2, load_system(modelRoot); else, error('SmartDebugger:ModelNotFound','Model not found: %s',model); end
        ...
        function ports=instrumentPorts(obj,block,direction,tx)
            ...
            if strcmpi(direction,'Inport'), handles=ph.Inport; displayDirection='Input'; else, handles=ph.Outport; displayDirection='Output'; end
            for k=1:numel(handles)
                p=template; p.Port=k; p.SignalHandle=handles(k); p.Name=sprintf('%s %d',displayDirection,k);
                try, p.LineHandle=get_param(handles(k),'Line'); catch, p.LineHandle=-1; end
                if isempty(p.LineHandle)||p.LineHandle==-1, ports(end+1,1)=p; continue; end
                ...
                if isempty(srcPort)||srcPort==-1, ports(end+1,1)=p; continue; end
                ...
                try
                    set_param(srcPort,'DataLogging','on','DataLoggingNameMode','Custom','DataLoggingName',p.LogName,'DataLoggingLimitDataPoints','off');
                catch ME
                    p.LogName=''; obj.Diagnostics.recordException(ME,sprintf('%s signal logging port %d',displayDirection,k));
                end
                ports(end+1,1)=p;
            end
        end
        ...
        function recordLoggingState(obj,root,inputs,outputs,logName)
            obj.Diagnostics.record('INFO','Logging configuration',sprintf('Model SignalLogging=%s, SignalLoggingName=%s, RunLogName=%s',obj.describeValue(obj.safeGetParam(root,'SignalLogging','')),obj.describeValue(obj.safeGetParam(root,'SignalLoggingName','')),logName),'SmartDebugger:LoggingConfiguration');
            obj.recordPortDiagnostics(inputs); obj.recordPortDiagnostics(outputs);
        end

        function recordPortDiagnostics(obj,ports)
            for k=1:numel(ports)
                if ports(k).LoggingHandle==-1, continue; end
                dl=obj.safeGetParam(ports(k).LoggingHandle,'DataLogging',''); nm=obj.safeGetParam(ports(k).LoggingHandle,'DataLoggingName','');
                obj.Diagnostics.record('INFO','Port logging configuration',sprintf('%s port %d: DataLogging=%s, DataLoggingName=%s',ports(k).Name,ports(k).Port,obj.describeValue(dl),obj.describeValue(nm)),'SmartDebugger:PortLoggingConfiguration');
            end
        end

        function logs=getLogs(~,simOut,logName)
            logs=[]; if isempty(simOut), return; end
        ...
        function ports=readLogged(obj,logs,ports)
            ...
            for k=1:numel(ports)
                p=ports(k);
                if isempty(p.LogName), out(end+1,1)=p; continue; end
                try
                    e=obj.findLogElement(logs,p.LogName);
                    if isempty(e), error('SmartDebugger:LoggedSignalMissing','Logged signal %s not found.',p.LogName); end
                    values=e.Values;
        ...
        function leaves=expandBusValues(obj,parent,values)
            ...
            data=[];
            try, data=values.Data; catch, return; end
            if ~isstruct(data), return; end
            fields=fieldnames(data);
            for k=1:numel(fields)
                child=data.(fields{k});
                childLeaves=obj.collectBusLeaf(parent,child,fields{k});
                if ~isempty(childLeaves), leaves=[leaves; childLeaves]; %#ok<AGROW> end
            end
            if ~isempty(leaves)
                obj.Diagnostics.record('INFO','Bus expansion',sprintf('%s expanded into %d leaf signals.',parent.Name,numel(leaves)),'SmartDebugger:BusExpansion');
            end
        end
        ...
        function leaves=collectBusLeaf(obj,parent,value,path)
            ...
            if obj.isTimeSeriesLike(value)
                ...
                p.Series=value;
                try, data=value.Data; p.Value=obj.lastSample(data); p.DataType=class(data); p.Dimension=mat2str(size(data)); p.SampleTime=obj.formatSampleTime(value.Time); obj.logRuntimeInfo(p.Name,value); catch, end
                leaves=p;
                return
            end
            if isstruct(value)
                ...
                for k=1:numel(fields)
                    childLeaves=obj.collectBusLeaf(parent,value.(fields{k}),[path '.' fields{k}]);
                    if ~isempty(childLeaves), leaves=[leaves; childLeaves]; %#ok<AGROW> end
                end
            end
        end
        ...
        function tf=isTimeSeriesLike(~,value)
            tf=false;
            try, tf=isa(value,'timeseries'); catch, end
            if tf, return; end
            try, tf=isobject(value)&&isprop(value,'Time')&&isprop(value,'Data'); catch, tf=false; end
        end

        function logRuntimeInfo(obj,name,series)
            try
                if ~isempty(series.Time)
                    obj.Diagnostics.record('INFO','Runtime capture',sprintf('%s: %d samples, t=%.12g..%.12g s, observed dt=%s',name,numel(series.Time),series.Time(1),series.Time(end),obj.formatSampleTime(series.Time)),'SmartDebugger:RuntimeSeries');
                end
            catch
            end
        end
        ...
        function element=findLogElement(~,logs,name)
            ...
            try
                for k=1:logs.numElements
                    e=logs.getElement(k);
                    try, if strcmp(char(e.Name),name), element=e; return; end, catch, end
                end
            catch
            end
        end
        ...
        function n=countCaptured(~,result)
            n=0; for k=1:numel(result.Inputs), if ~isempty(result.Inputs(k).Series), n=n+1; end, end; for k=1:numel(result.Outputs), if ~isempty(result.Outputs(k).Series), n=n+1; end, end
        end

        function t=firstTime(~,result)
            t=[]; for k=1:numel(result.Inputs), if ~isempty(result.Inputs(k).Series), t=result.Inputs(k).Series.Time(:); return; end, end; for k=1:numel(result.Outputs), if ~isempty(result.Outputs(k).Series), t=result.Outputs(k).Series.Time(:); return; end, end
        end

        function value=requiredGetParam(~,object,parameter)
            try, value=get_param(object,parameter); catch ME, error('SmartDebugger:RequiredModelParameter','Cannot read model parameter %s: %s',parameter,ME.message); end
        end
        function value=safeGetParam(~,object,parameter,defaultValue)
            try, value=get_param(object,parameter); catch, value=defaultValue; end
        end
        ...
        function text=simulationErrorMessage(~,simOut)
            text=''; if isempty(simOut), return; end
            try, if isprop(simOut,'ErrorMessage'), text=char(string(simOut.ErrorMessage)); end; catch, end
        end
        function message=formatSimulationError(~,message,stopTime,root)
            message=char(string(message)); low=lower(message);
            ...
            try, message=[message sprintf(' Model StopTime currently configured as %s.',char(string(get_param(root,'StopTime'))))]; catch, end
        end
        function tf=isAutoStopTime(~,stopTime)
            s=strtrim(char(string(stopTime))); tf=isempty(s)||strcmpi(s,'auto')||strcmpi(s,'auto (model)');
        end
        function value=lastSample(~,data)
            if isempty(data), value=[]; return; end
            try
                if isvector(data), value=data(end); else, subs=repmat({':'},1,ndims(data)); subs{1}=size(data,1); value=data(subs{:}); end
            catch, value=data(end); end
        end
        function text=formatSampleTime(~,t)
            ...
            dt=median(d); r=round(dt,6); if abs(dt-r)<1e-10, dt=r; end; text=sprintf('%.12g s',dt);
        end
    end
end