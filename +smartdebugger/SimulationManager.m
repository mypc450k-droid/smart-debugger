classdef SimulationManager < handle
    ...
    % TargetLink MIL is sensitive to the actual model SignalLogging
    % parameter. The debugger enables that parameter for the run, uses the
    % model's existing logging name, instruments source output ports, and
    % restores all changes afterward.
    ...
    methods (Access = private)
        function result = run(obj, model, block, stopTime, mode)
            ...
            % These variables must exist even when an early setup operation
            % fails. This prevents a secondary undefined-variable error.
            simOut = [];
            logs = [];
            ...
            try
                ...
                % TargetLink activates MIL capabilities before the actual
                % simulation and checks this model parameter directly.
        ...
        function root = ensureModelLoaded(~, model, block)
            ...
            if ~isempty(root)
                try, if bdIsLoaded(root), return; end, catch, end
            end
            ...
            for k=1:numel(handles)
                ...
                try
                    set_param(srcPort,'DataLogging','on','DataLoggingNameMode','Custom', ...
                        'DataLoggingName',p.LogName,'DataLoggingLimitDataPoints','off');
                catch ME
                    p.LogName='';
                    obj.Diagnostics.recordException(ME,sprintf('%s signal logging port %d',displayDirection,k));
                end
                ports(end+1,1)=p; %#ok<AGROW>
            end
        end
        ...
        function recordLoggingState(obj,root,inputs,outputs,logName)
            obj.Diagnostics.record('INFO','Logging configuration',sprintf( ...
                'Model SignalLogging=%s, SignalLoggingName=%s, RunLogName=%s', ...
                obj.describeValue(obj.safeGetParam(root,'SignalLogging','')), ...
                obj.describeValue(obj.safeGetParam(root,'SignalLoggingName','')),logName), ...
        ...
        function ports = readLogged(obj,logs,ports)
            ...
            for k=1:numel(ports)
                ...
                try
                    e=obj.findLogElement(logs,ports(k).LogName);
                    if isempty(e), error('SmartDebugger:LoggedSignalMissing','Logged signal %s not found.',ports(k).LogName); end
                    ts=e.Values; ports(k).Series=ts;
                    data=ts.Data; ports(k).Value=obj.lastSample(data); ports(k).DataType=class(data); ports(k).Dimension=mat2str(size(data));
                    if ~isempty(ts.Time)
                        ports(k).SampleTime=obj.formatSampleTime(ts.Time);
                        obj.Diagnostics.record('INFO','Runtime capture',sprintf('%s: %d samples, t=%.12g..%.12g s, observed dt=%s', ...
                            ports(k).Name,numel(ts.Time),ts.Time(1),ts.Time(end),ports(k).SampleTime),'SmartDebugger:RuntimeSeries');
                    end
                catch ME
        ...
        function t=firstTime(~,result)
            t=[];
            for k=1:numel(result.Inputs), if ~isempty(result.Inputs(k).Series), t=result.Inputs(k).Series.Time(:); return; end; end
            for k=1:numel(result.Outputs), if ~isempty(result.Outputs(k).Series), t=result.Outputs(k).Series.Time(:); return; end; end
        end
        ...
        function message=formatSimulationError(~,message,stopTime,root)
            ...
            if contains(low,'tpt test is still running')||contains(low,'stop time smaller than the length of the tpt test')
                if isempty(strtrim(stopTime))||strcmpi(strtrim(stopTime),'auto')||strcmpi(strtrim(stopTime),'auto (model)'), message=[message ' Smart Debugger did not override StopTime. Increase the TPT test-frame/model StopTime to cover the complete test case.']; else, message=[message ' Smart Debugger explicitly used StopTime=' stopTime '. Use auto or a value covering the complete TPT test case.']; end
            end
        ...
        function value=lastSample(~,data)
            ...
            subs=repmat({':'},1,ndims(data)); subs{1}=size(data,1); value=data(subs{:});
        end
